...
The first plugin AI Forge ships with (bundled, free) is one plugin on the platform, not a core feature. Give it a date window and a milestone; it produces a grounded release-post changelog for WordPress Core + Gutenberg.
...
Core commit messages carry everything a release post needs: `Fixes #NNNNN` (the closed Trac ticket), `Props alice, bob` (contributors), and `git-svn-id: …@62815` (the changeset `r62815`). The git mirror therefore gives reliable counts and links **without touching Trac**. For component grouping, each windowed ticket is joined against the dev-notes tracker (already tagged with a component and `dev-note` / `misc-dev-note` / `field-guide`). Tickets not in the tracker, newer than its snapshot, in excluded components, or version bumps, land under **Uncategorized**.

Everything the CLI needs is on GitHub: **cookie-free, deterministic, runs in CI and Codex.**

## Output

- **Default**: a technical report with a summary table plus the changelog grouped by area.
- **`--post`**: a release-post template with a headline, a count line, the two canonical source links (the Gutenberg compare view and the Core Trac query), a highlights placeholder, and the grouped changelog as raw material.
